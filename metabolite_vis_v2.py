...
def heatMapTableFromData(data, group_id, cmap, vmin=-2, vmax=2):
    ...
    table_data = []
    table_style = [('GRID', (0, 0), (-1, -1), 1, "white"), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
...
# ANALYTE_META_DATA = pd.read_csv("C:\\Users\\nmatulionis\\Desktop\\analyte_classes_philic_fixed.csv", dtype={0: 'str', 1: 'str', 2: 'str'})
# ANALYTE_META_DATA = pd.read_csv("C:\\Users\\ESchmid\\Desktop\\RESOURCES\\master_metabolite_standards_files\\analyte_classes.csv", dtype={0:'str', 1:'str', 2:'str'})
...
GLOBAL_ABBRV_MAP = {"phosphogylcerate": "PG",
...
class MetaboliteReport:
    ...
    def __init__(self, title, iso_df, ps_df, num_groups=-1, group_names={}, show_strct=True, show_iso=True,
                 show_ps=True):
        ...
        self.doc = SimpleDocTemplate(title + ".pdf",
                                     pagesize=letter,
                                     rightMargin=30,
                                     leftMargin=30,
                                     topMargin=30,
    ...
    def isotopomer_plot_image_for_data(self, data, error):
        ...
        data.plot(ax=ax, kind="bar",
                  stacked=True, rot=0,
                  width=0.9, edgecolor="white", linewidth=1, yerr=error, capsize=2,
                  color=(
                      "#d9d9d9",
                      "#a6d2de",
                      "#5a8e9c",
    ...
    def box_plot_image_for_data(self, data, title, ymax=100, yname=""):
        ...
        if p_val < 0.001:
            ...
            border_width = 2
        # else:
        # plt.text((self.num_groups + 1)*0.5, 0.5*ymax, "not significant", horizontalalignment='center',verticalalignment='center', alpha=0.35, fontweight='normal', fontsize=25, color="black", bbox={'facecolor': 'white', 'alpha':0.1, 'pad': 1, 'edgecolor': 'none'})
    ...
    def heatmap_for_data(self, data, title):
        ...
        ax = fig.add_subplot(111)
        cb = mp.colorbar.ColorbarBase(ax=ax, cmap=cmap, norm=Normalize(vmin=min_value, vmax=max_value),
    ...
    def table_of_plots_for_metabolite(self, met_name):
        ...
        if self.show_iso:
            ...
            iso_avg, iso_error = getGroupedAverageDataFromData(self.source_data_iso, self.num_groups, met_name,
                                                               self.group_names)
            if iso_avg.empty == False:
                table_row.append(self.isotopomer_plot_image_for_data(iso_avg, iso_error))

            frac_data = getFractionalContributionDataFromIsoData(self.source_data_iso, self.num_groups, met_name,
                                                                 self.group_names)
            print(frac_data)
            if frac_data.empty == False:
                table_row.append(self.box_plot_image_for_data(frac_data, title="Fractional Contribution", ymax=105,
                                                              yname="Fractional Contribution (%)"))

        pool_data = getPoolSizeFromData(self.source_data_pool, self.num_groups, met_name, self.group_names)
        if pool_data.empty == False:
            table_row.append(
                self.box_plot_image_for_data(pool_data, title="Pool Size", ymax=(1.2 * (np.nanmax(pool_data.values))),
                                             yname="Normalized Intensity"))
    ...
    def build_with_metabolite_classes(self, met_classes):
        ...
        if "title" in self.title_params:
            self.page_elements.append(Paragraph('<b>' + self.title_params["title"] + '</b>', STYLE_TITLE))
            self.page_elements.append(VGAP_04)
            self.page_elements.append(HR_LINE)
            self.page_elements.append(Spacer(0, 0.7 * inch))

        if "project" in self.title_params:
            self.page_elements.append(Paragraph('<b>Project:  ' + self.title_params["project"] + '</b>', STYLE_H1))
            self.page_elements.append(Spacer(0, 1 * inch))

        if "leader" in self.title_params:
            self.page_elements.append(Paragraph('<b>' + self.title_params["leader"] + '</b>', STYLE_H4))
            self.page_elements.append(Spacer(0, 0.15 * inch))

        if "exp_title" in self.title_params:
            self.page_elements.append(Paragraph('<b>' + self.title_params["exp_title"] + '</b>', STYLE_H4))
            self.page_elements.append(Spacer(0, 0.15 * inch))

        if "date" in self.title_params:
            self.page_elements.append(Paragraph('<b>Date:  ' + self.title_params["date"] + '</b>', STYLE_H4))
            self.page_elements.append(VGAP_04)

        if "exp_summary" in self.title_params:
            self.page_elements.append(Paragraph(self.title_params["exp_summary"], STYLE_H4))

        self.page_elements.append(Spacer(0, 1 * inch))

        self.page_elements.append(Paragraph(
        ...
        if "normalization" in self.title_params:
            ...
            if len(norms) > 0:
                self.page_elements.append(Spacer(0, 1.5 * inch))
                self.page_elements.append(Paragraph('<b>Pool sizes were normalized to field(s):</b>', STYLE_H4))

                for n in norms:
                    self.page_elements.append(Paragraph(n, STYLE_H4))

        print("___________________FINISHED_TITLE_PAGE_________________________")

        self.page_elements.append(PageBreak())

        self.page_elements.append(Paragraph('<b> Pool Sizes </b>', STYLE_TITLE_MIN))
        self.page_elements.append(VGAP_04)
        ...
        for met_class in met_classes:
            ...
            self.page_elements.append(group)
        ...
        for met_class in met_classes:
            ...
            for met in mets:
                ...
                if table != None:
                    ...
                    if ratio_value == False:
                        header = Paragraph(met, STYLE_MET_NAME)
                    else:
                        header = Paragraph(met, STYLE_MET_RATIO_NAME)

                    self.page_elements.append(KeepTogether([header, table]))
                    self.page_elements.append(VGAP_04)
                    print("_________________PROCESSED > " + met)

            if page_created == True: self.page_elements.append(PageBreak())

    def output_build_to_folder(self, folder_path):
...
def loadPreviousEntry():
    with open('previous_data.json', 'r') as json_file:
        ...
        description_box.insert("end-1c", data["previous entry"][0]["description_box"])
...
tk.Button(window, text="Generate Report", command=generateReport).grid(sticky="e", row=6, column=1, padx=10, pady=10)
tk.Button(window, text="Save Entry", command=savePreviousEntry).grid(sticky="e", row=7, column=1, padx=5, pady=5)
